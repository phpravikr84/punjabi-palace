 ...
 //Trial Balannce
    ...
    public function trial_balance_report(){
       ...
       if ($results['WithOpening']) {
            ...
            file_put_contents('assets/data/pdf/Trial_Balance_With_Opening_As_On_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf', $output);


            $data['pdf']    = 'assets/data/pdf/Trial_Balance_With_Opening_As_On_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf';
            ...
            $dompdf->render();
            $output = $dompdf->output();
            file_put_contents('assets/data/pdf/Trial_Balance_As_On_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf', $output);
            $data['pdf']    = 'assets/data/pdf/Trial_Balance_As_On_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf';
	...
	public function vouchar_view($id){
		$this->permission->method('accounts','read')->redirect();
        $vouchar_view = $this->accounts_model->get_vouchar_view2($id);
        $data = array(
			'vouchar_view' => $vouchar_view,
        );
		
		$data['vtitle'] = "Voucher Details for -".$id;
        $data['title'] = display('accounts_form');
        $data['module'] = "accounts";
        $data['page']   = "vouchar_cash";
        echo Modules::run('template/layout', $data);
...
//alhassan working
   ...
   public function coa_print(){
    ...
    public function profit_loss_report_search(){
        ...
        $data['dtpFromDate']  = $dtpFromDate;
        $data['dtpToDate']    = $dtpToDate;
        $data['pdf']    = 'assets/data/pdf/Statement_of_Comprehensive_Income_From_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf';
        $data['title']  = display('profit_loss');
        $data['module'] = "accounts";
    ...
    public function cash_flow_report_search(){
        ...
        $dompdf->render();
        $output = $dompdf->output();
        file_put_contents('assets/data/pdf/Cash_Flow_Statement_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf', $output);

        $data['pdf']    = 'assets/data/pdf/Cash_Flow_Statement_'.$dtpFromDate.'_To_'.$dtpToDate.'.pdf';
        $data['title']  = display('cash_flow');
        $data['module'] = "accounts";
