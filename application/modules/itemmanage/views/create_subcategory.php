        ...
        <div class="panel panel-bd lobidrag">
            <div class="panel-heading">
                <div class="panel-title">
                    <h4><?php echo (!empty($title) ? $title : 'Create Subcategory'); ?></h4>
                    <small style="font-size:0.7em; font-weight:400">(Note: Select a parent category for the subcategory. Fields marked with * are required.)</small>
                </div>
            </div>
            <div class="panel-body">
                <?php echo form_open_multipart("itemmanage/item_category/create_subcategory"); ?>

                <?php echo form_hidden('id', $this->session->userdata('id')); ?>
                <?php echo form_hidden('CategoryID', (!empty($categoryinfo->CategoryID) ? $categoryinfo->CategoryID : null)); ?>
                <?php echo form_hidden('type', 'subcategory'); ?>
                ...
                <div id="categoryContainer">
                            ...
                            <div class="category-row row mb-3 align-items-end border-bottom pb-3">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="Parentcategory_<?php echo $categoryIndex; ?>">Parent Category *</label>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label class="category-label">Subcategory Name *</label>
                                        ...
                                        <input name="categoryname[]" class="form-control category-input" type="text" 
                                            placeholder="Subcategory Name" value="<?php echo $sub->Name; ?>" required>
                                    </div>
                                </div>
                                <div class="col-md-1">
                                    <div class="form-group">
                    ...
                    <?php } else { ?>
                        <div class="category-row row mb-3 align-items-end border-bottom pb-3">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="Parentcategory_1">Parent Category *</label>
                                    <select name="Parentcategory[]" id="Parentcategory_1" class="form-control select2" required>
                                        <option value="">Select Parent Category</option>
                                        <?php foreach ($categories as $category) { ?>
                                            <option value="<?php echo $category->CategoryID; ?>">
                                                <?php echo $category->Name; ?>
                ...
                <div class="form-group row mb-3">
                    <div class="col-sm-12">
                        <button type="button" id="addCategory" class="btn btn-info"><?php echo 'Add SubCategory'; ?></button>
                    </div>
                </div>
...
$(document).ready(function () {
    ...
    $('form').submit(function (e) {
        ...
        $('.category-row').each(function(index) {
            ...
            if (!categoryInput) {
                errors.push(`Subcategory name cannot be empty in row ${index + 1}`);
            } else if (categoryNames.has(categoryInput)) {
                errors.push(`Duplicate subcategory name: ${categoryInput} in row ${index + 1}`);
            } else {
                categoryNames.add(categoryInput);
            }

            if (!groupSelect) {
                errors.push(`Please select a parent category in row ${index + 1}`);
            }
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    swal({
                        title: "Error",
                        text: response.message || "Failed to delete subcategory",
                        type: "error"
                    });
                }
            },
