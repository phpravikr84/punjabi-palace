...
$(document).ready(function() {
    ...
    function applyRounding(price, roundingMethod, roundToNearest) {
        ...
        if (!roundingMethod || roundingMethod === 'none') {
            return basePrice.toFixed(2); // No rounding
        }

        let lastDigit = (basePrice % 0.10) * 100; // Get cents
    ...
    function updateTablePrices($scheduleRow) {
        ...
        $('#itemTableBody tr').each(function() {
            ...
            let priceKey = 'price'; // Default to item price
            if (basedOn === 'sp') {
    ...
    // Trigger initial change events for Select2
                ...
                success: function(data) {
                    ...
                    $.each(data, function(i, item) {
                        ...
                        let itemData = {
                            ...
                            prices: {
                                '1': String(item.price || '0.00'), // Assuming customer_type_id '1' for Dine-In
                                '2': String(item.takeaway_price || '0.00'), // Assuming '2' for Takeaway
                                '3': String(item.uber_eats_price || '0.00') // Assuming '3' for UberEats
                            },
                            cost_price: String(item.cost_price || '0.00') // Assuming cost_price is available
                        };
        ...
        $('.item-checkbox:checked').each(function() {
            ...
            try {
                ...
                $('#itemTableBody').append(`
                    <tr data-prices='${JSON.stringify(item.prices)}' data-cost-price="${item.cost_price}">
    ...
    $('#saveButton').on('click', function(e) {
        ...
        if (!priceLevel) {
            e.preventDefault();
            alert('<?php echo $this->lang->line('price_level_required'); ?>');
        } else if (items === 0) {
            e.preventDefault();
