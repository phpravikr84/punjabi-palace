	...
	public function create($id = null)
	{
		...
		if ($this->form_validation->run()) {
			...
			foreach ($categoryNames as $index => $categoryName) {
				$parentId = !empty($parentCategories) ? implode(',', $parentCategories) : 0;
				$isOffer = isset($isOffers[$index]) ? 1 : 0;
				$offerStartDate = $isOffer ? date('Y-m-d', strtotime($this->input->post("offerstartdate", true))) : "0000-00-00";
				$offerEndDate = $isOffer ? date('Y-m-d', strtotime($this->input->post("offerendate", true))) : "0000-00-00";
			...
			if (!empty($insertedCategories)) {
				$logData = [
					'action_page' => "Add Category",
					'action_done' => "Insert Data",
					'remarks' => "Multiple categories created",
					'user_name' => $this->session->userdata('fullname', true),
					'entry_date' => date('Y-m-d H:i:s'),
				];
				$this->logs_model->log_recorded($logData);
