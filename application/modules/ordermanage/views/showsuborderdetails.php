  <thead>
    ...
    <tbody>
        ...
        $totalvat = isset($totalvat) ? $totalvat : 0;
        $itemprice = 0;
        $SD = isset($SD) ? $SD : 0;
        $pvat = 0;
        $multiplletax = isset($multiplletax) ? $multiplletax : array();
        ...
        if (!empty($iteminfo)) {
            foreach ($iteminfo as $item) {
                ...
                $itempricesingle = $item->price * ($presenttab[$item->row_id] ?? 0);
                ...
                if (!empty($item->add_on_id) && !empty($isaddones)) {
                    ...
                    foreach ($addons as $addonsid) {
                        ...
                        $adonsprice += $adonsinfo->price * $addonsqty[$y];
                        $tax = 0;
                        if (!empty($taxinfos)) {
                            foreach ($taxinfos as $taxainfo) {
                                ...
                                if (!empty($adonsinfo->$fildaname)) {
                                    ...
                                    $multiplletax[$fildaname] = ($multiplletax[$fildaname] ?? 0) + $avatcalc; 
                                } else {
                ...
                <tr>
                    ...
                    <td><?php echo ($presenttab[$item->row_id] ?? 0); ?></td>
                    <td>
                        <?php 
                        if ($item->OffersRate > 0) { 
                            $discountt = ($item->price * $item->OffersRate) / 100;  
                            $itemtotal = ($presenttab[$item->row_id] ?? 0) * $item->price - (($presenttab[$item->row_id] ?? 0) * $discountt) + $adonsprice;
                            echo number_format($itemtotal, 3);
                            $totalprice += $itemtotal;
                            $itemprice = $itemtotal;
                        } else {
                            $itemtotal = $adonsprice + ($presenttab[$item->row_id] ?? 0) * $item->price;
                            echo number_format($itemtotal, 3);
                            $totalprice += $itemtotal;
                            $itemprice = $itemtotal;
                        }
					...
					<?php if($item->status ==0){ ?>
						<td>
							<button type="button" id="del_split_<?php echo $item->row_id; ?>" class="btn btn-danger btn-sm text-light">
            ...
            ?>
            <tr>
                <td colspan="6" class="text-center">No items in this suborder</td>
            </tr>
            <?php 
        }
        ...
        if ($settinginfo->service_chargeType == 1) {
            ...
            $count = !empty($suborder_info) ? count($suborder_info) : 1;
    ...
    <tfoot>
            ...
            foreach ($taxinfos as $mvat) {
                if ($mvat['is_show'] == 1) {
                    ...
                    <tr>
                        ...
                        <td class="text-right"><b><?php echo number_format($multiplletax['tax' . $i], 3); ?></b></td>
                    </tr>
                    <?php 
                    $i++;
                }
        ...
        <tr>
            ...
            <td class="text-right"><b><?php echo number_format($totalprice + $calvat + $service_chrg_data, 3); ?></b></td>
