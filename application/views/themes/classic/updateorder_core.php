...
<head>
    ...
    <link href="<?php echo base_url();?>application/views/themes/<?php echo $acthemename; ?>/assets_web/css/updateorder_core.css" rel="stylesheet">
                            ...
                            <div class="price_area">
                        		...
                        		<input type="hidden" name="cartpage" id="cartpage_<?php echo $item->CategoryID.$k;?>" value="1">
                         		<input name="itemprice" type="hidden" value="<?php echo $item->price;?>" id="itemprice_<?php echo $item->CategoryID.$k;?>" />
									 ...
									 <?php $myid2=$item->ProductsID.$item->variantid;
									if(!empty($iteminfo)){
											$allid2="";
											foreach ($iteminfo as $cartitem){
											    $updateproduct=$cartitem->menu_id.$cartitem->varientid;
												if($updateproduct==$myid2){ $allid2.=$myid2.",";?>
                                                <button class="simple_btn d-none" id="backadd<?php echo $item->CategoryID.$k;?>" onClick="addonsitemqr('<?php echo $item->ProductsID;?>','<?php echo $item->variantid;?>',<?php echo $item->CategoryID.$k;?>,<?php echo $orderinfo->order_id;?>)">
                                    <span>add</span>
                                </button>
                                    <div class="cart_counter active" id="removeqtyb<?php echo $item->CategoryID.$k;?>">
                                    <input type="text" name="qty" id="sst<?php echo $item->CategoryID.$k;?>" maxlength="12" value="<?php echo $cartitem->menuqty;?>" title="Quantity:" class="input-text qty" readonly>
                                    <button id="<?php echo $item->CategoryID.$k;?>" onclick="addextra('<?php echo $item->ProductsID;?>','<?php echo $item->variantid;?>',<?php echo $item->CategoryID.$k;?>,<?php echo $orderinfo->order_id;?>)" class="increase items-count" type="button">
										...
										$myvalue2=trim($str2,',');
										if($myid2!=$myvalue2){?>
										<button class="simple_btn" id="backadd<?php echo $item->CategoryID.$k;?>" onClick="addonsitemqr('<?php echo $item->ProductsID;?>','<?php echo $item->variantid;?>',<?php echo $item->CategoryID.$k;?>,<?php echo $orderinfo->order_id;?>)">
                                    <span>add</span>
                                </button>
                                <div class="cart_counter hidden_cart" id="removeqtyb<?php echo $item->CategoryID.$k;?>">
                                    <input type="text" name="qty" id="sst<?php echo $item->CategoryID.$k;?>" maxlength="12" value="1" title="Quantity:" class="input-text qty" readonly>
                                    <button id="<?php echo $item->CategoryID.$k;?>" onclick="addextra('<?php echo $item->ProductsID;?>','<?php echo $item->variantid;?>',<?php echo $item->CategoryID.$k;?>,<?php echo $orderinfo->order_id;?>)" class="increase items-count" type="button">
                                        <i class="fa fa-plus"></i>
                                    </button>
                                </div>
                        		...
                        		<input type="hidden" name="cartpage" id="cartpage_<?php echo $item->CategoryID.$k;?>" value="1">
                         		<input name="itemprice" type="hidden" value="<?php echo $item->price;?>" id="itemprice_<?php echo $item->CategoryID.$k;?>" />
                               	<?php $myid=$item->ProductsID.$item->variantid;
								if(!empty($iteminfo)){	
														
									$allid="";
									foreach ($iteminfo as $cartitem){
										$updateproduct=$cartitem->menu_id.$cartitem->varientid;		
										if($updateproduct==$myid){ $allid.=$myid.","; //echo $myid;?>
                                ...
                                <?php }else if($updateproduct==$myid){}?>
                                        
										<?php  }
										$str = implode(',',array_unique(explode(',', $allid)));
										$myvalue=trim($str,',');
										if($myid!=$myvalue){?>
										<button class="simple_btn" id="backadd<?php echo $item->CategoryID.$k;?>" onClick="addextra('<?php echo $item->ProductsID;?>','<?php echo $item->variantid;?>',<?php echo $item->CategoryID.$k;?>)">
                                    <span>add</span>
                                </button>
                                <div class="cart_counter hidden_cart" id="removeqtyb<?php echo $item->CategoryID.$k;?>">
                                    <input type="text" name="qty" id="sst<?php echo $item->CategoryID.$k;?>" maxlength="12" value="1" title="Quantity:" class="input-text qty" readonly>
                ...
                <div class="col-12">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="cart_sm" id="cartitemandprice">
                           <h6 class="mb-0"><?php echo $orderinfo->totalamount;?></h6>
                        </div>
                        <a href="<?php echo base_url();?>qr-menu" class="btn mr-2">Go to Menu<i class="fa fa-angle-double-right"></i></a>
                    </div>
                </div>
            </div>
        </div>
...
<script src="<?php echo base_url(); ?>application/views/themes/<?php echo $acthemename; ?>/assets_web/js/qrapp.js"></script>
</body>
